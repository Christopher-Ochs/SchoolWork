...
def group_by_centroid(restaurants, centroids):
    ...
    for i in restaurants:
        x = find_closest(restaurant_location(i), centroids)
        rest.append([x, i])
    return group_by_first(rest)

def find_centroid(cluster):
    """Return the centroid of the locations of the restaurants in cluster."""
    x_location = mean([restaurant_location(i)[0] for i in clsuter])
    y_location = mean([restaurant_location(i)[1] for i in cluster])
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(i) for i in clsuters]
        n += 1
    return centroids
